/**
*
* Test Script Name                      : Customer_TC_0576_VerifyApplicationConsumeTransactionFromAnnualTierAndPriceFromServiceBundleWhenUseMyCredentialIsDisabledInConnectivityTab
* TestDataSheet                         : Customer_TC_0576
* Objective                             : Verify whether the Application consume transaction from annual tier and price from service bundle when 'Use my credential' is disabled.
* Version                               : 1.0
...
public class Customer_TC_0576_VerifyApplicationConsumeTransactionFromAnnualTierAndPriceFromServiceBundleWhenUseMyCredentialIsDisabledInConnectivityTab {

	// DECLARATION SECTION
	  static String TestDataTab="Customer_TC_0576";
	  ...
	  DBUtil DBUtil;
	    ...
	    public Customer_TC_0576_VerifyApplicationConsumeTransactionFromAnnualTierAndPriceFromServiceBundleWhenUseMyCredentialIsDisabledInConnectivityTab(Map<Object, Object> testdatamap){
	    ...
	    public void myTest() throws InterruptedException, Exception{
	        try{
				...
				Pages.editServiceProviderListPage.clickOnConnectivityTypeToggleIfTheToggleIsOFF(Log);
				...
				Pages.customerProfilePage.clickOnServiceProviderName(testdatamap,Log);
				Pages.customerProfilePage.clickOnUseMyCredentialsToggleInConnectivityTabIfToggleIsON(testdatamap,Log);
				Pages.customerProfilePage.clickOnHomeTab();
				Pages.customerPackageManagementPage.searchAnnualPackageNameWithRespectiveStatus(testdatamap,Log);
				String transactionConsumptionValue=Pages.customerPackageManagementPage.getTransactionConsumption();
				// Parse the String to an integer
				int intValue = Integer.parseInt(transactionConsumptionValue);
                // Increment the integer value by 1
				intValue++;
				// Convert the incremented integer back to String
				String transConsumptionValueAfterExecutingService = Integer.toString(intValue);


				Pages.customerPackageManagementPage.searchServiceBundlePackageNameWithRespectiveStatus(testdatamap,Log);
				String priceConsumptionValue=Pages.customerPackageManagementPage.getPriceConsumptionValue();
				// Parse the String to an integer
				int intValue1 = Integer.parseInt(priceConsumptionValue);
				// Increment the integer value by 8
				intValue1+=8;
				// Convert the incremented integer back to String
				String priceConsumptionValueAfterExecutingService = Integer.toString(intValue1);
				...
				Pages.customerServicesPage.enterRequestBody(testdatamap,Log);
				Pages.customerServicesPage.clickOnExecute(Log);
				Pages.customerServicesPage.verifyApiResponseStatusCode(testdatamap,Log);
				Pages.customerProfilePage.clickOnHomeTab();
				Pages.customerPackageManagementPage.searchAnnualPackageNameWithRespectiveStatus(testdatamap,Log);
				Pages.customerPackageManagementPage.verifyTransactionConsumptionDataWhenUseMyCredentialsIsEnabled(transConsumptionValueAfterExecutingService,Log);

				Pages.customerPackageManagementPage.searchServiceBundlePackageNameWithRespectiveStatus(testdatamap,Log);
				Pages.customerPackageManagementPage.verifyPriceConsumptionDataWhenUseMyCredentialsIsEnabled(priceConsumptionValueAfterExecutingService,Log);
